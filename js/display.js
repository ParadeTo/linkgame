var LinkGame = function (x, y, z, l, dom) {
  this.x = x //列数
  this.y = y //行数
  this.l = 100 / 100 //游戏满盈率，最大为1(表示没有空白)，需要注意x*y*l%z=0
...
LinkGame.prototype = {
  ...
  gamearrmap: function () {
    ...
    for (var m = 0; m < that.z; m++) {
      for (var n = 0; n < max; n++) arrbase[n + m * max] = n + 1
    }
    ...
    for (var o = 0; o < arrbase.length; o++) {
      arrmap[Math.floor(arrorder[o] / that.x) + 1][(arrorder[o] % that.x) + 1] =
        arrbase[o]
    }
  ...
  renderdom: function () {
    ...
    for (var i = 0; i < that.y; i++) {
      for (var j = 0; j < that.x; j++) {
        that.dom.append(
          '<li class=list' + that.arrmap[i + 1][j + 1] + '><i></i></li>'
        )
      }
    }
    that.dom.append(
